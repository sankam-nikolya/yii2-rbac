...
class Helper
{
    ...
    /**
     * Filter action column button. Use with [[sankam\core\adminlte\widgets\GridView]]
     * ```php
     * 'columns' => [
     *     ...
     *     [
     ...
     *     [
     *         'class' => 'yii\grid\ActionColumn',
     *         'template' => Helper::filterActionColumn(['view','update', ['activate' => '/other-controller/action']]),
     *          'buttons' => [
    ...
    public static function filterActionColumn($buttons = [], $user = null)
    {
        if (is_array($buttons)) {
            ...
            foreach ($buttons as $button) {
                ...
                if(is_array($button)) {
                    $route = reset($button);
                    $button = reset(array_keys($button));
                }

                if (static::checkRoute($route, [], $user)) {
                    $result[] = "{{$button}}";
                }
            }
