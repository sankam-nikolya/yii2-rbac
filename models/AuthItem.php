...
namespace sankam\rbac\models;

use sankam\rbac\components\Configs;
use sankam\rbac\components\Helper;
use Yii;
use yii\base\Model;
...
class AuthItem extends Model
{
    ...
    public function rules()
    {
        return [
            ...
            [['name'], 'unique', 'when' => function () {
                return $this->isNewRecord || (mb_strtolower($this->_item->name) != mb_strtolower($this->name));
            }],
            [['type'], 'integer'],
    ...
    public function getItems()
    {
        ...
        foreach (array_keys($manager->getPermissions()) as $name) {
            $available[$name] = ($name[0] == '/' || $name[0] == '@') ? 'route' : 'permission';
        }
        ...
        foreach ($manager->getChildren($this->_item->name) as $item) {
            if($item->type == 1) {
                ...
                $assigned[$item->name] = ($item->name[0] == '/' || $item->name[0] == '@') ? 'route' : 'permission';
            }
    ...
    {
        $result = [
            Item::TYPE_PERMISSION => Yii::t('rbac-admin', 'Permission'),
            Item::TYPE_ROLE => Yii::t('rbac-admin', 'Role'),
        ];
        if ($type === null) {
            return $result;
        }
    ...
    public static function checkPrimaryGroup($name = null)
    {
        $groups = [
            Configs::defaultUserGroup(),
            Configs::defaultAdminGroup()
        ];
